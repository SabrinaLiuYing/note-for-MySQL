...
-- partition
SELECT state, AVG(points) OVER (PARTITION BY state)
FROM customers;

-- CTE: Commen Table Expression
-- cte only exist in memory
...
-- Trim, LTRIM, RTRIM
 SELECT EmployeeID, TRIM(EmployeeID) AS ETRIM
-- SELECT EmployeeID, LTRIM(EmployeeID) AS ETRIM
-- SELECT EmployeeID, RTRIM(EmployeeID) AS ETRIM
FROM EmployeeErrors;
...
-- substring
SELECT DISTINCT SUBSTRING(FirstName,1,3) AS New_first_name
FROM employeeerrors;

-- UPPER and lower
SELECT 
	*, 
	UPPER(FirstName) AS upper_first, 
    LOWER(LastName) AS lower_last
FROM employeeerrors;
